...
class EncuestasController extends ControladorBase{
		...
		if (isset(  $_SESSION['nombre_usuarios']) )
		{
			...
			if (!empty($resultPer))
			{
				...
				$columnas="pre.nombre_preguntas_encuestas_participes,
						  ...
						  CASE  WHEN pre.id_preguntas_encuestas_participes=3 THEN  (select count(*) from public.encuentas_participes_detalle enc where enc.id_preguntas_encuestas_participes=pre.id_preguntas_encuestas_participes and enc.id_preguntas_encuestas_participes=3 and enc.respuestas_encuestas_participes='La Informaci贸n') END LA_INFORMACION_3,
						  CASE  WHEN pre.id_preguntas_encuestas_participes=3 THEN  (select count(*) from public.encuentas_participes_detalle enc where enc.id_preguntas_encuestas_participes=pre.id_preguntas_encuestas_participes and enc.id_preguntas_encuestas_participes=3 and enc.respuestas_encuestas_participes='Las Im谩genes') END LAS_IMAGENES_3,
	...
	public function print_enc()
	{
		...
		$fechaactual = getdate();
		$dias = array("Domingo","Lunes","Martes","Miercoles","Jueves","Viernes","S谩bado");
			...
			if(isset($_GET["id_encuestas"])){
					...
					if(!empty($resultEncuestas_Cabec)){
						...
						if($_id_encuentas_participes_cabeza > 0 && $_id_usuarios > 0){
							...
							$html.= '<p style="margin-top:15px; text-align: justify; font-size: 13px;"><b>NOMBRES:</b> '.$_nombre_usuarios.'  <b style="margin-left: 20%; font-size: 13px;">IDENTIFICACI脫N:</b> '.$_cedula_usuarios.'</p>';
	                        ...
	                        if (!empty($resultSet)){
		                        ...
		                        elseif ($numero==3){
		                        	...
		                        	if($respuesta=='La Informaci贸n'){
		                        		 
		                        		$html.='<th style="text-align: center;  font-size: 15px; font-weight: normal;"><b><u>La Informaci贸n</u></b></th>';
		                        		 
		                        	}else{
		                        		$html.='<th style="text-align: center;  font-size: 15px; font-weight: normal;">La Informaci贸n</th>';
		                        		 
		                        	}
		                        	
		                        	if($respuesta=='Las Im谩genes'){
		                        		 
		                        		$html.='<th style="text-align: center;  font-size: 15px; font-weight: normal;"><b><u>Las Im谩genes</u></b></th>';
		                        		 
		                        	}else{
		                        		$html.='<th style="text-align: center;  font-size: 15px; font-weight: normal;">Las Im谩genes</th>';
	...
	public function buscaAspirante(){
	    ...
	    if($action == 'ajax')
	    {
	        ...
	        $adjacents  = 9; //brecha entre p醙inas despu閟 de varios adyacentes
	...
	public function imprime_encuesta()
	{
	    ...
	    $fechaactual = getdate();
	    $dias = array("Domingo","Lunes","Martes","Miercoles","Jueves","Viernes","S醔ado");
	    $meses = array("Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre");
	    $fechaactual=$dias[date('w')]." ".date('d')." de ".$meses[date('n')-1]. " del ".date('Y') ;
        ...
        if(isset($_GET["id_aspirante"])){
            ...
            if(!empty($resultEncuestas)){
                ...
                $html.='<p style="text-align: right;">'.$logo.'<hr style="height: 2px; background-color: black;"></p>';
                $html.='<p style="text-align: right; font-size: 13px;"><b>Impreso:</b> '.$fechaactual.'</p>';
                $html.='<p style="text-align: center; font-size: 16px;"><b>PROCESO DE SELECCI覰</b></p>';
                
                $html.= '<p style="margin-top:15px; text-align: justify; font-size: 13px;"><b>NOMBRES:</b> '.$_nombre_aspirante.'  <b style="margin-left: 20%; font-size: 13px;">IDENTIFICACI覰:</b> '.$_cedula_aspirante.'</p>';
                $html.= '<p style="margin-top:15px; text-align: justify; font-size: 13px;"><b>ENCUESTA REALIZADA EL:</b> '.$_fecha_encuesta.'</p>';
